...
public class OpcUaClientConnector implements SessionActivityListener {
  ...
  public CompletableFuture<UaClient> connect(String url) {
    ...
    OpcUaClientConfig config =
        OpcUaClientConfig.builder().setApplicationName(LocalizedText.english(name))
    ...
    client.get().addFaultListener(
        fault -> logger.error("fault on {}", fault.getResponseHeader().getServiceResult()));
  ...
  public CompletableFuture<UaSubscription> modify(UaSubscription subscription,
      double publishingInterval, int lifetimeCount, int maxKeepAliveCount, int maxNotifications,
      byte prio) throws InterruptedException, ExecutionException {
    UInteger subscriptionId = subscription.getSubscriptionId();
    UInteger requestedLifetimeCount = uint(lifetimeCount);
    UInteger requestedMaxKeepAliveCount = uint(maxKeepAliveCount);
    UInteger maxNotificationsPerPublish = uint(maxNotifications);
    UByte priority = UByte.valueOf(prio);
    return client.get().getSubscriptionManager().modifySubscription(subscriptionId,
        publishingInterval, requestedLifetimeCount, requestedMaxKeepAliveCount,
        maxNotificationsPerPublish, priority);
  }

  public CompletableFuture<UaSubscription> subscribe(ReferenceDescription rd)
    ...
    Optional<UaSubscription> subItem = client.get().getSubscriptionManager().getSubscriptions()
        .stream().filter(s -> s.getMonitoredItems().stream()
            .anyMatch(m -> m.getReadValueId().getNodeId().equals(nodeId)))
        .findFirst();
    ...
    UaSubscription subscription =
        client.get().getSubscriptionManager().createSubscription(DEFAULT_PUBLISH_INTERVAL).get();
    UInteger clientHandle = uint(clientHandles.getAndIncrement());
    MonitoringParameters parameters = new MonitoringParameters(clientHandle,

    DEFAULT_PUBLISH_INTERVAL, // sampling interval
  ...
  public CompletableFuture<UaSubscription> unsubscribe(UaSubscription subscription) {
    ...
    logger.debug("remove subscriptionId: {}", subscription.getSubscriptionId());
    return client.get().getSubscriptionManager()
  ...
  public void unsubscribeAll() {
    ...
    UnmodifiableIterator<UaSubscription> it =
        client.get().getSubscriptionManager().getSubscriptions().iterator();
  ...
  public CompletableFuture<Tuple2<ServerState, ZonedDateTime>> readServerStateAndTime() {
    if (client.get() == null) {
      return buildCompleteExceptionally(new CompletableFuture<Tuple2<ServerState, ZonedDateTime>>(),
  ...
  public CompletableFuture<BrowseResult> getHierarchicalReferences(NodeId node) {
    ...
    BrowseDescription bd = new BrowseDescription(node, BrowseDirection.Forward,
        Identifiers.HierarchicalReferences, true, nodeClassMask, resultMask);
    return browse(bd);
  }

  public ReferenceDescription getRootNode(String displayName) {
    return new ReferenceDescription(Identifiers.RootFolder, Boolean.TRUE,
        Identifiers.RootFolder.expanded(), QualifiedName.parse("Root"),
  ...
  public CompletableFuture<List<DataValue>> read(NodeId node, AttributeId attr) {
    ...
    return client.get().read(0.0, TimestampsToReturn.Both, Collections.singletonList(node),
  ...
  public CompletableFuture<StatusCode> write(WriteValue value) {
    ...
    return client.get().write(Collections.singletonList(value)).thenApply(WriteResponse::getResults)
        .thenApply(d -> d[0]);
  }
  ...
  public CompletableFuture<StatusCode> writeValue(NodeId node, DataValue value) {
    ...
    return client.get()
        .writeValues(Collections.singletonList(node), Collections.singletonList(value))
        .thenApply(d -> d.get(0));
  }
  ...
  private <T> CompletableFuture<T> buildCompleteExceptionally(CompletableFuture<T> cf,
      Throwable th) {
    cf.completeExceptionally(th);
    return cf;
  }
