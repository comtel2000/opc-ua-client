...
public enum NodeIdLookup {
  ...
  public static Optional<NodeIdLookup> find(int id){
    return Arrays.stream(values()).filter(v -> v.id == id).findAny();
  }
  
  public static NodeIdLookup lookup(int id){
    return Arrays.stream(values()).filter(v -> v.id == id).findAny().orElse(NodeIdLookup.Unknown);
  }
}
