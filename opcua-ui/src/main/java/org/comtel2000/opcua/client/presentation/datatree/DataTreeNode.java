...
public class DataTreeNode extends TreeItem<ReferenceDescription> {
	...
	result.thenApply(r -> Arrays.stream(r.getReferences()).map(this::createNode).collect(Collectors.toList())).whenCompleteAsync(this::updateChildren, Platform::runLater);
